...
def from_contour(contour: Contour, context: Context) -> Box:
    return _from_iterable(contour.vertices, context)


def from_contours(contours: Iterable[Contour], context: Context) -> Box:
    return _from_iterable(flatten(contour.vertices for contour in contours),
                          context)


def from_multipolygon(multipolygon: Multipolygon, context: Context):
    return from_contours((polygon.border for polygon in multipolygon.polygons),
                         context)


def from_multisegment(multisegment: Multisegment, context: Context) -> Box:
    return _from_iterable(flatten((segment.start, segment.end)
                                  for segment in multisegment.segments),
                          context)


def from_polygon(polygon: Polygon, context: Context):
    return from_contour(polygon.border, context)


def from_segment(segment: Segment, context: Context) -> Box:
    start, end = segment.start, segment.end
    return context.box_cls(min(start.x, end.x), max(start.x, end.x),
                           min(start.y, end.y), max(start.y, end.y))


def _from_iterable(points: Iterable[Point], context: Context) -> Box:
